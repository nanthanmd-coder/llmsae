...
def sample_vlsae(
    ...
    **model_kwargs,
    ...
    model_kwargs_sae = model_kwargs.copy() # copy model_kwargs for cd only for the first forward process
    ...
    while True:
        ...
        use_vlsae = model_kwargs.get("use_sae")
        ...
        if use_vlsae:
            ...
            feature_extractor = model_kwargs.get("feature_extractor")
            feature_extractor.set_model(self)
            feature_extractor.set_vis_indices(model_kwargs.get("vis_indices"))
            target_layer = model_kwargs.get("target_layer")
            feature_extractor.add_hooks(target_layer)
            outputs_sae = self(
                **model_inputs,
                return_dict=True,
                output_attentions=output_attentions,
            ...
            feature_extractor.remove_hooks(target_layer)
            next_token_logits_sae = outputs_sae.logits[:, -1, :]
            ...
            # diffs = (1+cd_alpha)*next_token_logits - cd_alpha*next_token_logits_sae
            diffs = (1-cd_alpha)*next_token_logits + cd_alpha*next_token_logits_sae
            ...
            if cd_logits.max() == -float("inf"):
                ...
                # print(torch.sum(cd_logits!=-float("inf")))
        ...
        if use_vlsae:
            model_kwargs_sae = self._update_model_kwargs_for_generation(
                outputs_sae, model_kwargs_sae, is_encoder_decoder=self.config.is_encoder_decoder
            )
...
def evolve_vlsae_sampling():
    transformers.generation.utils.GenerationMixin.sample = sample_vlsae